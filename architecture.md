...
- Any audio STT model may add `+cleanup` (and diarizing models may compose it as `+names+cleanup`). After STT and optional name mapping, `transcript-cleanup.js` asks Gemini for a bounded JSON correction list (`{from, to, context?}`) based on participant names, active drafts, and slide titles. Corrections are normalized with strict safety guards (dropping conflicting mappings, script/charset mismatches, and unanchored common English words) and applied with word-boundary matching and phrase context anchoring (`applyCorrections` returning `{ text, appliedCount, applied }`). This preserves timestamps and all uncorrected content by construction and fails soft. The cleaned result uses the existing session-ID transcript cache key; switching cleanup modes on a cached session requires `--uncache <selector> --uncache-type transcripts`.
